...
async function getTodos(req, res) {
  ...
  try {
    ...
    return res.send({
      status: 200,
      message: 'berhasil mendapatkan daftar todo',
      todos,
    });
  } catch (err) {
...
async function updateTodoById(req, res) {
  ...
  try {
    await todoServices.updateTodoById(id, todoId, title, isCompleted);
    return res.send({ message: 'successfully updated todo', status: 200 });
  } catch (error) {
    return res.status(400).send({ message: 'failed to update todo', status: 400 });
  }
}
